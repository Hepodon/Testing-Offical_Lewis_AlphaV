...
namespace lewis {
inline namespace v5 {
class Drivetrain : public virtual pros::AbstractMotor {
public:
  Drivetrain(
  ...
  Drivetrain(
      ...
      const pros::v5::MotorGears gearset = pros::v5::MotorGears::invalid,
      const pros::v5::MotorUnits encoder_units = pros::v5::MotorUnits::invalid);
  Drivetrain(AbstractMotor &motor_group);

  std::int32_t drive(std::int32_t voltage) const;

  std::int32_t drive_absolute(const double position,
